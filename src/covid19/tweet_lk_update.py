...
def _get_tweet_text():
    ...
    dnew_confirmed = new_confirmed - new_confirmed_wa
    new_confirmed_arw = '🔴' if (dnew_confirmed > 0) else '🟢'

    new_deaths_rw = sum(ts_new_deaths[-MW:]) / MW
    new_deaths_rw_wa = sum(ts_new_deaths[-MW - MW: -MW]) / MW
    dnew_deaths = new_deaths_rw - new_deaths_rw_wa
    new_deaths_ar = '🔴' if (dnew_deaths > 0) else '🟢'

    new_vax_rw = (ts_cum_vaxs[-1] - ts_cum_vaxs[-MW - 1]) / MW
    new_vax_rw_wa = (ts_cum_vaxs[-1 - MW] - ts_cum_vaxs[-MW - 1 - MW]) / MW
    dnew_vacci = new_vax_rw - new_vax_rw_wa
    new_vax_ar = '🟢' if (dnew_vacci > 0) else '🔴'
    ...
    dnew_pcr_tests = new_pcr_tests_rw - new_pcr_tests_rw_wa
    new_pcr_tests_ar = '🟢' if (dnew_pcr_tests > 0) else '🔴'

    tweet_text = '''{date} #COVID19SL

{new_confirmed_arw} Cases: {new_confirmed:,} ({dnew_confirmed:+,} {MW}days ago)
{new_deaths_ar} Deaths/day: {new_deaths_rw:,.0f} ({dnew_deaths:+,.0f})
{new_pcr_tests_ar} Tests/day: {new_pcr_tests_rw:,.0f} ({dnew_pcr_tests:+,.0f})
{new_vax_ar} Vaxs/Day: {new_vax_rw:,.0f} ({dnew_vacci:+,.0f})
...
@HPBSriLanka @JHUSystems @OurWorldInData #lka #SriLanka
    '''.format(
        ...
        new_confirmed_arw=new_confirmed_arw,
        new_vax_rw=new_vax_rw,
        date=date,
        dnew_vacci=dnew_vacci,
        dnew_confirmed=dnew_confirmed,
        dnew_deaths=dnew_deaths,
